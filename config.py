...
class Config(object):
    TG_BOT_TOKEN = os.environ.get("TG_BOT_TOKEN", "6321355601:AAF_m1wBN2niavAIK6Yt6kNoimdEC4lJS5A") # Make a bot from https://t.me/BotFather and enter the token here
    #If deploying on vps edit the above value as example := TG_BOT_TOKEN = "Your-bot-token-inside-inverted-commas."
    
    APP_ID = int(os.environ.get("APP_ID", 28939232)) # Get this value from https://my.telegram.org/apps
    #If deploying on vps edit the above value as example := APP_ID = Your-APP_ID-without-inverted-commas
    
    API_HASH = os.environ.get("API_HASH", "74bd38f61e669fac1be09e9eca4cd431") # Get this value from https://my.telegram.org/apps
    ...
    Bot_username = os.environ.get("Bot_username", "6321355601:AAF_m1wBN2niavAIK6Yt6kNoimdEC4lJS5A") # Your bot's telegram username (must enter with '@' in the front of the username)
    #If deploying on vps edit the above value as example := Bot_username = "Your-Bot_username-inside-inverted-commas."
    
    OWNER_ID = os.environ.get("OWNER_ID", 1880221341) # Your(owner's) telegram id
    ...
    AUTH_USERS = set(int(x) for x in os.environ.get("AUTH_USERS", "1040033572").split()) # Id's of the telegram users, who you want to allow for multitasking - downloading multiple links at once!
